...
Transfer(okCFrontPanel *dev, UINT32 u32Count, bool bWrite)
{
	...
	if (((dev->GetWireOutValue(0x3E) & 0x1) != 0x1) && (m_ePattern == OK_PATTERN_FIXED)) {
		printf("Fixed pattern is not supported by this bitstream. Switching to LFSR.\n");
...
{
	printf("Usage: %s bitfile [serial S] [repeat N] [pattern lfsr|sequential|walking1|walking0|hammer|neighbor|fixed F]\n", progname);
	...
	printf("   bitfile        - Configuration file to download, alternatively specify \"nobit\" to skip configuration.\n");
	printf("   repeat N       - Repeats the requested tests for N seconds (\"inf\" to run forever).\n");
	...
	printf("      fixed F     - Selects a fixed pattern defined by hex input F. (USB 3.0 only)\n");
...
real_main(int argc, char *argv[])
{
	const char* serial;
	UINT32 i, orig_i;
	int pipetest_start_time, current_time;
...
#if defined(_WIN32) || defined(__QNX__)
	pipetest_start_time = clock() / NUM_CPS;
#else
	gettimeofday(&a, 0);
	pipetest_start_time = (a.tv_sec * 1000000 + a.tv_usec) / NUM_CPS;
#endif

	if ((argc > i) && !strcmp(argv[i], "repeat")) {
	...
	current_time = -1;
	orig_i = i;
	while ((current_time < (rep_time + pipetest_start_time)) || run_forever) {
		for (; i<(UINT32)argc; i++) {
...
#if defined(_WIN32) || defined(__QNX__)
		current_time = clock() / NUM_CPS;
#else
		gettimeofday(&a, 0);
		current_time = (a.tv_sec * 1000000 + a.tv_usec) / NUM_CPS;
#endif
		i = orig_i;
	}
